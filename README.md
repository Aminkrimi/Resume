...
- **Interactive terminal** (`` ` ``) with `git log` / `git branch` / `git status`, `neofetch`, `perf`, `cat <file>` (prints a section's source), `ls`, `source`, `inspect` and more, plus the **command palette** (`⌘K` / `Ctrl+K` / `/`).
